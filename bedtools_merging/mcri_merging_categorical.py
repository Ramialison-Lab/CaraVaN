...
# READ THE ORIGINAL FILE JUST DOWNLOADED + FILTERING + INCREMENT
...
print("READING THE ORIGINAL FILE AND FILTERINING + INCREMENT")



# read the original file
df = pd.read_csv(f"Validation/homo_sapiens-{chromosome}.vcf", sep = '\t', skiprows = 36)
...
def append_chr(x):
   return 'chr'+str(x)
df['chrom'] = df['chrom'].apply(lambda x: append_chr(x))

#### interrupt for filtering SNVs here: already done
df = df[df['INFO'].str.contains("TSA=SNV")]
df = df.drop(['ID', 'FILTER', 'REF', 'ALT', 'QUAL', 'INFO'], axis = 1)


print(chromosome)
print(positions)
# add the orange variants to the list
if positions:
    positions = [int(ele) for ele in positions]
    #  bug here
    res = df[df['POS'].isin(positions)]
    positions = [ele for ele in positions if ele not in res['POS'].tolist()]
    # bug ends here?
    print(positions)

    if positions:
        length = len(positions)
        df_chrom = [chromosome] * length
        df_positions = positions
        df_positions_incr = [ele + 1 for ele in positions]
        new_df = pd.DataFrame({'chrom': df_chrom, 'POS': df_positions, 
                               'POS_INCR': df_positions_incr})
        df = pd.concat([df, new_df], ignore_index = True)
        df = df.drop_duplicates(subset=["chrom", "POS"])
        df = df.sort_values(by = ['chrom', 'POS', 'POS_INCR'], 
                            ascending = [True, True, True])

print("ADDING NEW POSITIONS DONE")
print("STARTING TO SAVE TO VALIDATION FOLDER")
# add the parser to add more variants here
