# Library declaration
# for inference_others --> interrupt in mcri_merging_categorical.py
import numpy as np
import pandas as pd
...
logging_file = f'logging/{CHROMOSOME}_inference_during_running_human_encode.txt'
...
full_pipeline = joblib.load(f'model_weights/full_pipeline_{label_text}.joblib')
print("LOADED FULL PIPELINE")

pca = joblib.load(f'model_weights/pca_{label_text}.joblib')
print("LOADED FULL PCA")

clf_full = joblib.load(f'model_weights/clf_full_{label_text}.joblib')
...
positions.to_csv(f"predictions/proba_predictions/{CHROMOSOME}_scores_{label_text}_others.txt", index = False, sep = '\t')
