...
# label_text in ['human_encode', 'tfbs']
...
########## declare some features for reading and parsing purposes
...
########### start the creating labelling process --> data + label
features = master_copy[master_copy.columns[29:]]
screen_encode_features = list(features.columns[:42])
human_encode_samples = list(screen_encode_features[:-8])
mouse_encode_samples = list(screen_encode_features[-8:])
zebrafish_features = list(features.columns[42:43]) # done
print(zebrafish_features)
# ncer_features = list(features.columns[43:44]) # done
# tfbs_features = list(features.columns[44:120])
# dickel_features = list(features.columns[120:121]) # done
# vista_features = list(features.columns[121:122]) # done
# cvdkp_features = list(features.columns[122:152])
# hic_features = list(features.columns[152:155])
# # pchic_features = features.columns[154:155]
# atac_features = list(features.columns[155:193])
...
mapping_dict_label = {
  "human_encode": human_encode_samples,
  "tfbs": tfbs_features
}

cols_as_labels = mapping_dict_label[label_text]
implementation = mapping_dict[label_text]


def create_label(master, cols_as_labels, cols_to_drop = []):
  if len(cols_as_labels) == 1:
    label = master[cols_as_labels]
  else:
    label = master[cols_as_labels].sum(axis = 1).map(lambda x: 0
                                                     if x == 0 else 1)

  master_cop = master.copy()
  master_cop = master_cop[master_cop.columns[29:]]
  master_cop = master_cop.drop(cols_as_labels, axis = 1)
  master_cop = master_cop.drop(cols_to_drop, axis = 1)
  return master_cop, label


features, label = create_label(master_copy, cols_as_labels = cols_as_labels,
                               cols_to_drop = zebrafish_features)

# very important: parsing so that it follows the order
...
def train_model(name,  x_train, y_train, tune = False):
  if name == 'lgbm':
    ...
    if not tune:
        ...
        clf3 = CatBoostClassifier(custom_loss="Logloss", scale_pos_weight=18,
           logging_level='Silent', random_state = 42)

        model = VotingClassifier(estimators=[('catboost', clf3), ('logistic', clf1), ('lgbm', clf2)], voting = 'soft')
        model.fit(x_train, y_train)
        return model
...
# saving weights
import pickle
with open(f"model_weights/ensemble_{label_text}.pkl", 'wb') as f:
  pickle.dump(clf_full, f)
with open(f"model_weights/pca_voting_{label_text}.pkl", 'wb') as fp:
  pickle.dump(pca, fp)
joblib.dump(full_pipeline, f'model_weights/pipeline_{label_text}.pkl')
...
def get_scores_proba(model_full, x_train_full,  x_test_full,  y_train, y_test):
    ...
    plt.savefig(f"plots/voting_{label_text}_confusion.png", bbox_inches = 'tight', dpi = 600)
    ...
    plt.legend()
    plt.savefig(f"plots/voting_{label_text}_roc.png", bbox_inches = 'tight', dpi = 600)
